...
const USE_BACKEND_UPLOAD = import.meta.env.VITE_USE_BACKEND_UPLOAD === "true";

/**
 * Upload image to Cloudinary
 * First tries backend endpoint for signed uploads (if enabled), otherwise direct unsigned upload
...
export const uploadImageToCloudinary = async (
  ...
  if (USE_BACKEND_UPLOAD) {
    try {
      ...
      console.warn(
        "Backend upload failed, falling back to direct upload:",
        backendError
      );
    }
  }
  ...
  if (!CLOUD_NAME) {
    throw new Error(
      "Cloudinary cloud name not configured. Set VITE_CLOUDINARY_CLOUD_NAME in your frontend environment."
    );
  }
  if (!UPLOAD_PRESET) {
    throw new Error(
      "No upload preset configured. Set VITE_CLOUDINARY_UPLOAD_PRESET for unsigned uploads or enable backend-signed uploads."
    );
  }
 ...
 * @returns {Promise<{url: string, public_id: string, filename: string, size: number, type: string}>}
 */
